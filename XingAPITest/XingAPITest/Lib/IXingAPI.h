...
typedef enum {
	...
	STATE_REQUEST_DATA,
	STATE_MESSAGE_DATA,
	STATE_SYSTEM_ERROR_DATA,
	STATE_RELEASE_DATA,
	STATE_LINK_DATA = 10		// XM_RECEIVE_LINK_DATA 메시지의 구분 플래그
} STATE_RECV_DATA;
...
{
public:
	IXingAPI() { m_hModule = NULL;	m_hwd = NULL; ZeroMemory(this, sizeof(IXingAPI)); }
	~IXingAPI() { Uninit(); }
...
public:
	// 초기화 관련
	inline	BOOL	Init					( HWND m_hWnd, LPCSTR szPath=NULL );
	inline	BOOL	IsInit					()											{ return m_hModule != NULL; }
	inline	void	Uninit					();

	// 서버연결
	inline	BOOL	Connect					( LPCSTR pszSvrIP, int nPort, int nStartMsgID, int nTimeOut=-1, int nSendMaxPacketSize=-1 );
	inline	BOOL	IsConnected				();
	inline	BOOL	Disconnect				();
	
	// 로그인
	inline	BOOL	Login					( LPCSTR pszID, LPCSTR pszPwd, LPCSTR pszCertPwd, int nType, BOOL bShowCertErrDlg=TRUE );
	inline	BOOL	Logout					( void );

	inline	int		GetLastError			();
	inline	char*	GetErrorMessage			( int nErrorCode );
	
	// 조회성TR 관련
	inline	int		Request					( LPCSTR pszCode, LPVOID lpData, int nDataSize, BOOL bNext=FALSE, LPCSTR pszNextKey=NULL, int nTimeOut=30 );
	inline	BOOL	ReleaseRequestData		( int nRequestID );
	inline	BOOL	ReleaseMessageData		( LPARAM lp );

	// 실시간TR 관련
	inline	BOOL	AdviseRealData			( LPCSTR pszTrNo, LPCSTR pszData, int nDataUnitLen );
	inline	BOOL	UnadviseRealData		( LPCSTR pszTrNo, LPCSTR pszData, int nDataUnitLen );
	inline	BOOL	UnadviseWindow			( void );
	...
	// 정보얻기
	inline	char* GetCommMedia			();			// 통신매체를 구한다.
	inline	char* GetETKMedia			();			// 당사매체를 구한다.
	inline	char* GetClientIP			();			// 공인IP를 구한다.
	inline	char* GetServerName			();			// 서버명을 구한다.
	inline	char* GetAPIPath			();			// 실행중인 xingAPI의 경로
	...
	inline	char*   GetProcBranchNo			();			// 처리점을 구한다.
	...
	// 
	// 
	inline	int		RequestService			( LPCSTR pszCode, LPCSTR pszData );	
	...
	//
	inline	int		RemoveService			( LPCSTR pszCode, LPCSTR pszData );
	...
	inline	int		RequestLinkToHTS        ( LPCTSTR pszLinkKey, LPCTSTR pszData, LPCTSTR pszFiller=NULL );
	...
	inline	void	AdviseLinkFromHTS		( void );
	inline	void	UnAdviseLinkFromHTS		( void );
...
BOOL IXingAPI::Init(HWND m_hWnd, LPCSTR szPath)
{
	if (IsInit())
	{
		printf("Already Initalize Done.\n");
		return TRUE;
	}
	...
	if (m_hwd  == NULL)
	{
		printf("m_hwd is NULL.\n");
		return FALSE;
	}

	//if ((m_hwd = GetConsoleWindow()) == NULL)
	//{
	//	printf("m_hwd is NULL.\n");
	//	return FALSE;
	//}
...
{
	char* path = (char*)szPath;
	//char* path = ConvertWCtoC((TCHAR*)szPath);
	...
	//printf("set current DIR to %s\n", path);

	m_hModule = ::LoadLibrary(_T("xingAPI.dll"));
...
BOOL IXingAPI::Connect( LPCSTR pszSvrIP, int nPort, int nStartMsgID, int nTimeOut, int nSendMaxPacketSize )
{
	ASSERT( m_hModule );
	if( NULL == m_hModule || NULL == m_hwd) return FALSE;
	if( NULL == m_fpConnect ) m_fpConnect = (FP_CONNECT)GetProcAddress(m_hModule, "ETK_Connect");
	ASSERT( m_fpConnect );
	if( NULL == m_fpConnect ) return FALSE;

	return m_fpConnect(m_hwd, pszSvrIP, nPort, nStartMsgID, nTimeOut, nSendMaxPacketSize );
}
...
BOOL IXingAPI::Login( LPCSTR pszID, LPCSTR pszPwd, LPCSTR pszCertPwd, int nType, BOOL bShowCertErrDlg )
{
	ASSERT( m_hModule );
	if( NULL == m_hModule || NULL == m_hwd)	return FALSE;
	...
	return m_fpLogin( m_hwd, pszID, pszPwd, pszCertPwd, nType, bShowCertErrDlg );
}

BOOL IXingAPI::Logout( void )
{
	ASSERT( m_hModule );
	if( NULL == m_hModule || NULL == m_hwd) return FALSE;
...
char* IXingAPI::GetErrorMessage( int nErrorCode )
{
	ASSERT( m_hModule );
	if( NULL == m_hModule  ) return NULL;
	...
	if( NULL == m_fpGetErrorMessage ) return NULL;

	//char szData[1024] = { 0 };
	char* szData = (char*)ESmalloc(sizeof(char)*1024);
...
int IXingAPI::Request(LPCSTR pszCode, LPVOID lpData, int nDataSize, BOOL bNext, LPCSTR pszNextKey, int nTimeOut )
{
	if( NULL == m_hModule || NULL == m_hwd)	return -1;
	...
	return m_fpRequest(m_hwd, pszCode, lpData, nDataSize, bNext, pszNextKey, nTimeOut);
}

BOOL IXingAPI::AdviseRealData ( LPCSTR pszTrNo, LPCSTR pszData, int nDataUnitLen )
{
	if( NULL == m_hModule || NULL == m_hwd)	return FALSE;
	...
	return m_fpAdviseRealData(m_hwd, pszTrNo, pszData, nDataUnitLen );
}

BOOL IXingAPI::UnadviseRealData( LPCSTR pszTrNo, LPCSTR pszData, int nDataUnitLen )
{
	if( NULL == m_hModule || NULL == m_hwd)	return FALSE;
	...
	return m_fpUnadviseRealData( m_hwd, pszTrNo, pszData, nDataUnitLen );
}

BOOL IXingAPI::UnadviseWindow( void )
{
	if( NULL == m_hModule || NULL == m_hwd)	return FALSE;
	...
	return m_fpUnadviseWindow(m_hwd);
}
...
char* IXingAPI::GetCommMedia(void)
{
	if( NULL == m_hModule )	return NULL;
	...
	if( NULL == m_fpGetCommMedia )	return NULL;

	TCHAR szMedia[256] = { 0 };
	m_fpGetCommMedia( szMedia );

	char* res = ConvertWCtoC(szMedia);

	return res;
}

char* IXingAPI::GetETKMedia(void)
{
	if( NULL == m_hModule )	return NULL;
	...
	if( NULL == m_fpGetETKMedia )	return NULL;
...
char* IXingAPI::GetClientIP(void)
{
	if( NULL == m_hModule )	return NULL;
	...
	if( NULL == m_fpGetClientIP )	return NULL;

	TCHAR szIP[256] = { 0 };
	m_fpGetClientIP( szIP );

	char* res = ConvertWCtoC(szIP);

	return res;
}

char* IXingAPI::GetServerName(void)
{
	if( NULL == m_hModule )	return NULL;
	...
	if( NULL == m_fpGetServerName )	return NULL;

	TCHAR szName[256] = { 0 };
	m_fpGetServerName( szName );

	char* res = ConvertWCtoC(szName);
	return res;
}

char* IXingAPI::GetAPIPath(void)
{
	if( NULL == m_hModule )	return NULL;
	...
	if( NULL == m_fpGetAPIPath )	return NULL;

	TCHAR szPath[256] = { 0 };
	m_fpGetAPIPath( szPath );

	char* res = ConvertWCtoC(szPath);
	return res;
}
...
char* IXingAPI::GetProcBranchNo(void)
{
	if( NULL == m_hModule )	return NULL;
	...
	if( NULL == m_fpGetProcBranchNo )	return NULL;

	TCHAR szProcBranchNo[256] = { 0 };
	m_fpGetProcBranchNo( szProcBranchNo );

	char* res = ConvertWCtoC(szProcBranchNo);
	return res;
}
...
int IXingAPI::RequestService( LPCSTR pszCode, LPCSTR pszData )
{
	if( NULL == m_hModule || NULL == m_hwd)	return -1;
	...
	return m_fpRequestService(m_hwd, pszCode, pszData );
}

int IXingAPI::RemoveService( LPCSTR pszCode, LPCSTR pszData )
{
	if( NULL == m_hModule || NULL == m_hwd)	return -1;
	...
	return m_fpRemoveService(m_hwd, pszCode, pszData );
}

int IXingAPI::RequestLinkToHTS( LPCTSTR pszLinkKey, LPCTSTR pszData, LPCTSTR pszFiller )
{
	if( NULL == m_hModule || NULL == m_hwd)	return -1;
	...
	return m_fpRequestLinkToHTS(m_hwd, pszLinkKey, pszData, pszFiller );
}

void IXingAPI::AdviseLinkFromHTS( void )
{
	if( NULL == m_hModule || NULL == m_hwd)	return;
	...
	m_fpAdviseLinkFromHTS(m_hwd);
}

void IXingAPI::UnAdviseLinkFromHTS( void )
{
	if( NULL == m_hModule || NULL == m_hwd)	return;
