...
typedef struct _ChartExcelInBlock
{
    ...
    char    indexouttype        [   1];    // [string,    1] 결과데이터 구분                 StartPos 90, Length 1
    char    market              [   1];    // [string,    1] 시장구분                        StartPos 91, Length 1
    char    period              [   1];    // [string,    1] 주기구분                        StartPos 92, Length 1
    char    shcode              [   8];    // [string,    8] 단축코드                        StartPos 93, Length 8
    char    isexcelout          [   1];    // [string,    1] 결과 지표데이터 엑셀표시 여부   StartPos 101, Length 1
    char    excelfilename       [ 256];    // [string,  256] 엑셀데이터 파일명               StartPos 102, Length 256
    char    IsReal              [   1];    // [string,    1] 실시간 데이터수신 자동등록 여부 StartPos 358, Length 1
